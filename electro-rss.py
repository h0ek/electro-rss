...
# --- NEW: normalize category to Film/Seriale ---
def normalize_category(cat: str) -> str:
    return 'Seriale' if cat.strip().lower() == 'seriale' else 'Film'
...
def fetch_items(days=7):
    ...
    def _one(cat, url):
        ...
        def _from_cache():
            return [it for it in prev_items
                    if (it.get('source', it.get('category')) == cat) and
